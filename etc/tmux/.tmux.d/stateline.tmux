...
if-shell "uname | grep -q Darwin" \
  'set-option -g status-right "#{prefix_highlight} temp: #{temp_cpu} | CPU: #{cpu_percentage} | #{sysstat_mem} | #{battery_status_bg} Batt: #{battery_icon} #{battery_percentage} #{battery_remain} | %m/%d %H:%M#[default]"' \
  'set-option -g status-right "#{prefix_highlight} temp: #{temp_cpu} | CPU: #{cpu_percentage} | #{sysstat_mem} | %m/%d %H:%M#[default]"'
